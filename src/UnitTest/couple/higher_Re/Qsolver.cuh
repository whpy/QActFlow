...
void r1p_init(Qreal *r1, Qreal dx, Qreal dy, int Nx, int Ny){
    for (int j=0; j<Ny; j++){
        for (int i=0; i<Nx; i++){
            ...
            r1[index] = 0.2*(sin(0.1*(x+y))*sin(0.1*(x+y)) - 0.5) + (double(rand())/RAND_MAX)*0.0001;
        }
    }
}
...
void r2p_init(Qreal *r2, Qreal dx, Qreal dy, int Nx, int Ny){
    for (int j=0; j<Ny; j++){
        for (int i=0; i<Nx; i++){
            ...
            float x = dx*i;
            float y = dy*j;
            // r2[index] = (double(rand())/RAND_MAX)*;
            r2[index] = 0.2*sin(0.1*(x+y))*cos(0.1*(x+y)) + (double(rand())/RAND_MAX)*0.0001;
        }
    }
}
