...
void NL1exact(Qreal* phys, Qreal lambda, Qreal Pe, Qreal cn, int Nx, int Ny, int BSZ, Qreal dx, Qreal dy){
    ...
    if (i<Nx && j<Ny){
        phys[index] = 3.*cos(2*x+3*y)*sin(2*x)
        + (13*sin(x)*sin(x) + 12*1.414213562*lambda*sin(x)*sin(x))*sin(2*x+3*y)
...
void NL2exact(Qreal* phys, Qreal lambda, Qreal Pe, Qreal cn, int Nx, int Ny, int BSZ, Qreal dx, Qreal dy){
    ...
    if (i<Nx && j<Ny){
        phys[index] = 3.*cos(2*x+3*y)*sin(2*x)
        + (-13.*sin(x)*sin(x) + 5.*1.414213562*lambda*sin(x)*sin(x))*sin(2*x+3*y)
        - 8*sin(x)*sin(x)*sin(x)*sin(x)*sin(x)*sin(x)*cn*cn/Pe;
    }
}
...
void NL0exact(Qreal* phys, Qreal lambda, Qreal Pe, Qreal Er, Qreal Re, Qreal cn, int Nx, int Ny, int BSZ, Qreal dx, Qreal dy){
    ...
    Qreal y = j*dy;
    if (i<Nx && j<Ny){
        phys[index] = 1.0/(Re*Er*sin(x)*sin(x))*
        ( -11.3137*lambda*cos(x)*cos(x)*cos(x)*cos(x)*sin(x)*sin(x)
        - 33.9411*lambda*sin(x)*sin(x)*sin(x)*sin(x)*sin(x)*sin(x)
        - 0.4*sin(x)*sin(x)*sin(x)*sin(x)
        + cos(x)*cos(x)*(135.765*lambda*sin(x)*sin(x)*sin(x)*sin(x) + 0.4*sin(x)*sin(x))
        + lambda*sin(x)*sin(x)*sin(x)*sin(x)*(11.3137*sin(x)*sin(x) - 181.019*sin(x)*sin(x)*sin(x)*sin(x)*sin(x)*sin(x) 
...
void Single1exact(Qreal* phys, Qreal lambda, Qreal cn, int Nx, int Ny, int BSZ, Qreal dx, Qreal dy){
    ...
    // Qreal s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = cos(x+y)*(0.2+4*lambda+6*lambda*cn*cn);
    }
}
...
void Single2exact(Qreal* phys, Qreal lambda, Qreal cn, int Nx, int Ny, int BSZ, Qreal dx, Qreal dy){
    ...
    Qreal s = exp(-1*( (x-M_PI)*(x-M_PI)+(y-M_PI)*(y-M_PI) ));
    if (i<Nx && j<Ny){
        phys[index] = sin(x+y)*(0.2+4*lambda+6*lambda*cn*cn);;
    }
}
...
int main(){
    ...
    Qreal a = 0.2;
    cout << "Re = " << Re << endl; cout << "Er = " << Er << endl;
    cout << "alpha = " << a << endl; cout << "Pe = " << Pe << endl;
    cout << "lambda = " << lambda << endl; cout << "Cn = " << cn << endl;
...
// the intermediate components p (p11, p12, p21)
    ...
    lambda, cn, Pe, Er, Re);
    FldSet<<<mesh->dimGridp, mesh->dimBlockp>>>(alpha->phys, a, Nx, Ny, BSZ);
